...
const LEGACY_SESSION_TO_SLOT = Object.freeze({
  ...
  signal: 'signal',
  evening: 'wrap',
  // v1 identity mappings (covers briefs published with v1 names but missing slot field)
  radar: 'radar',
  pulse: 'pulse',
  wrap: 'wrap',
...
// Per-slot "key numbers" highlights. Returns up to 4 {label, value, color}
// entries parsed from the brief body. Different metrics per slot — these are
// the headline numbers an agent reader would scan first per brief type.
//   radar:  Risk Gauge / Composite / Geopol / HY spreads (macro setup)
//   signal: CAPE / F&G / ICSA / 10Y BE (cross-asset prep)
//   pulse:  BTC / ETH / SOL / OI (crypto microstructure)
//   wrap:   SPY / QQQ / VIX / Sector dispersion (close)
export function extractBriefHighlights(brief, slot) {
  const body = String(brief.body || brief.telegramText || '').replace(/<[^>]+>/g, '');
  function mk(regex, label, fmt, color) {
    const m = body.match(regex);
    if (!m) return null;
    const raw = m[1];
    const value = fmt ? fmt(raw) : raw;
    return { label, value: String(value), color: color || '' };
  }
  function mkPct(ticker) {
    const re = new RegExp(ticker + '\\s+([+-]?[0-9.]+%|flat)');
    const m = body.match(re);
    if (!m) return null;
    const v = m[1];
    let c = '';
    if (v.startsWith('-')) c = 'red';
    else if (v.startsWith('+')) c = 'green';
    return { label: ticker, value: v, color: c };
  }
  switch (slot) {
    case 'radar': return [
      mk(/Risk Gauge:\s*([0-9.]+)\/100/, 'RISK', v => Math.round(parseFloat(v))),
      mk(/Composite Risk:\s*([0-9.]+)\/100/, 'COMPOSITE', v => parseFloat(v).toFixed(1)),
      mk(/Geopolitical risk:\s*([0-9.]+)\/100/, 'GEOPOL', v => Math.round(parseFloat(v))),
      mk(/HY Credit Spreads:\s*(\d+)bps/, 'HY SPREADS', v => v + 'bp'),
    ].filter(Boolean);
    case 'signal': return [
      mk(/CAPE:\s*([0-9.]+)/, 'CAPE'),
      mk(/Equity sentiment \(CNN F&G\):\s*([0-9.]+)/, 'F&G'),
      mk(/ICSA:\s*(\d+)K/, 'ICSA', v => v + 'K'),
      mk(/10Y breakeven\s*([0-9.]+)%/, '10Y BE', v => v + '%'),
    ].filter(Boolean);
    case 'pulse': return [
      mk(/BTC:\s*\$([0-9,]+)/, 'BTC', v => '$' + v, 'yellow-c'),
      mk(/ETH:\s*\$([0-9,]+)/, 'ETH', v => '$' + v, 'yellow-c'),
      mk(/SOL:\s*\$([0-9.]+)/, 'SOL', v => '$' + v, 'yellow-c'),
      mk(/Total OI \(top \d+ perps\):\s*\$([0-9.]+[BMK])/, 'OI', v => '$' + v),
    ].filter(Boolean);
    case 'wrap': return [
      mkPct('SPY'),
      mkPct('QQQ'),
      mkPct('VIX'),
      mk(/Sector dispersion:\s*(\d+)/, 'DISPERSION'),
    ].filter(Boolean);
    default: return [];
  }
}
