...
function renderCollection(opts) {
  ...
  const faqJsonLd = {
    ...
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: 'What is The Record?',
        acceptedAnswer: { '@type': 'Answer', text: 'A permanently archived daily intelligence feed. AgentCanary publishes four macro briefs every day (Radar 03:15, Signal 09:15, Pulse 15:15, Wrap 21:15 UTC) tracking regime states, top movers, scenarios, and signals. Every brief is timestamped and remains addressable forever.' } },
      { '@type': 'Question', name: 'How are predictions scored?',
        acceptedAnswer: { '@type': 'Answer', text: 'Conditional predictions extracted from each Signal Scan brief are evaluated against actual market prices 48-72 hours after publish. Aggregate hit / partial / miss counts are displayed on /record/.' } },
      { '@type': 'Question', name: 'Can AI agents consume this?',
        acceptedAnswer: { '@type': 'Answer', text: 'Yes. AgentCanary exposes a REST API at api.agentcanary.ai, an MCP server (npx agentcanary-mcp), JSON Feed at /record/feed.json, RSS at /record/rss.xml, and llms.txt for AI crawlers.' } },
      { '@type': 'Question', name: 'What does each brief cover?',
        acceptedAnswer: { '@type': 'Answer', text: 'Macro Radar (03:15 UTC) covers overnight macro and risk gauge. Signal Scan (09:15) covers cross-asset rotation and forward scenarios. Market Pulse (15:15) covers intraday crypto microstructure. Market Wrap (21:15) covers daily synthesis and contrarian setups.' } },
    ],
  };
  return wrapPage({
...
const SEO_FOOTER = `<section class="ac-seo-footer">
  <h2 class="ac-seo-footer-title">About AgentCanary</h2>
  <p>AgentCanary is cross-asset macro market intelligence built for AI agents and traders. We publish four time-stamped briefs every day — <strong>Macro Radar</strong> at 03:15 UTC (overnight setup), <strong>Signal Scan</strong> at 09:15 (cross-asset prep), <strong>Market Pulse</strong> at 15:15 (intraday crypto microstructure), and <strong>Market Wrap</strong> at 21:15 (close synthesis) — covering regime classification, risk gauge, forward scenarios, top movers, sector strength, whale flows, derivatives positioning, and intel themes. Every brief is permanently archived as a timestamped page; predictions are scored against actual market outcomes 48-72 hours later. Access via <a href="https://api.agentcanary.ai/api/docs">REST API</a>, <a href="https://www.npmjs.com/package/agentcanary-mcp">MCP server</a> (<code>npx agentcanary-mcp</code>), <a href="/record/feed.json">JSON Feed</a>, <a href="/record/rss.xml">RSS</a>, or the <a href="https://clawhub.ai/MrCerq/agentcanary">ClawHub skill</a>. AI crawlers welcomed via <a href="/llms.txt">llms.txt</a> and an explicit robots.txt allowlist.</p>
</section>`;

function wrapPage(opts) {
  const { title, metaDescription, canonical, prev, next, jsonLd, jsonLdExtra, breadcrumbHtml, bodyHtml, noindex, topRightHtml, leadHtml } = opts;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
