...
function _extractSnapshot(brief) {
  ...
  const m = body.match(/<b>SNAPSHOT<\/b>\s*([\s\S]*?)(?=<b>|$)/i);
...
function _dayMeta(day) {
  ...
  const headline = wrap ? (wrap.desc || wrap.headline || '') : '';
  let regime = '';
...
function renderCollection(opts) {
  ...
  // Today's 4-brief grid (one card per slot present that day)
  let todayCardsHtml = '';
  let todayDateLong = '';
  let latestPublished = '';
  if (today) {
    todayDateLong = _longDate(today.date);
    const dayUrl = `/record/${today.date.replace(/-/g, '/')}/`;
    todayCardsHtml = (today.briefs || []).map(brief => {
      const slot = resolveSlot(brief);
      const meta = slotMeta(slot);
      const permalink = briefPermalink(brief, slot);
      const excerpt = _extractSnapshot(brief);
      ...
      return `      <a href="${permalink}" class="rc-brief-card rc-slot-${escapeHtml(slot)}">
        <div class="rc-brief-head">
          ...
          <span class="rc-brief-time">${escapeHtml(meta.fireTimeUTC)}</span>
        </div>
    ...
    // Latest published — use the last brief's publishedAt or compute
    const lastBrief = (today.briefs || [])[today.briefs.length - 1];
    if (lastBrief) {
      const slot = resolveSlot(lastBrief);
      const meta = slotMeta(slot);
      latestPublished = `${_shortDate(today.date)}, ${today.date.slice(0, 4)} · ${meta.fireTimeUTC} UTC`;
    }
  }
...
<section class="rc-masthead">
  <div class="rc-container">
    ...
    <h1 class="rc-h1">Every brief, <em>on the record.</em></h1>
    ...
    <div class="rc-masthead-meta">
      ${latestPublished ? `<span><strong>Latest:</strong> ${escapeHtml(latestPublished)}</span>` : ''}
      <a href="/record/feed.json">JSON Feed</a>
      <a href="/record/rss.xml">RSS</a>
      <a href="https://t.me/AgentCanary">Telegram</a>
      <a href="https://api.agentcanary.ai/api/docs">API</a>
    </div>
  </div>
</section>
  ...
  <div class="rc-container">
    <div class="rc-section-head">
      <div class="rc-section-head-left">
        <div class="rc-section-eyebrow">Today · ${escapeHtml(todayDateLong)}</div>
        <h2 class="rc-h2">Today's briefs.</h2>
      </div>
...
function renderBrief(opts) {
  ...
  // Pull SNAPSHOT paragraph from body for articleBody excerpt (helps AI search
  // index the actual content, not just the headline)
  const bodyRaw = String(brief.body || brief.telegramText || brief.content || '');
  let articleBody = brief.desc || '';
  const snapMatch = bodyRaw.match(/<b>SNAPSHOT<\/b>\s*([\s\S]*?)(?=<b>|$)/i);
  if (snapMatch) {
    articleBody = snapMatch[1].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim().slice(0, 600);
  }
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    ...
    articleBody,
    image: 'https://agentcanary.ai/og-image.png',
