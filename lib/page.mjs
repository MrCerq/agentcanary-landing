...
function computeBriefScores(date, briefs, predictionsArr) {
  const slotLabels = {
    radar: 'MACRO RADAR',
    signal: 'SIGNAL SCAN',
    pulse: 'MARKET PULSE',
  ...
  return SLOT_ORDER.map(slot => {
    ...
    // radar/pulse/wrap: rubric exists but scoring pipeline not built
    return { slot, label, status: 'pending', summary: 'Brief-grading pipeline pending' };
  });
}

function renderDayScorecard(date, briefs, predictionsArr) {
  const scores = computeBriefScores(date, briefs, predictionsArr);
  ...
  const pill = (label, count, color) => `
    <div class="ac-agg-pill" style="border-color:${color};color:${color}">
...
function renderDay(opts) {
  ...
  return wrapPage({
    ...
    bodyHtml: `
      ...
      ${renderDayScorecard(date, sortedBriefs, opts.predictions || [])}
    `,
  });
}
  ...
  return wrapPage({
    ...
    bodyHtml: `
      <div class="ac-hero">
        ...
        <p class="ac-hero-sub">Brief published ${escapeHtml(meta.fireTimeUTC)} UTC</p>
      </div>
      <section class="ac-briefs-wrap">
        <div class="ac-timeline">
