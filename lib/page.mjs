...
function renderBreadcrumb(crumbs) {
  ...
  return `<nav class="ac-breadcrumb" aria-label="Breadcrumb" style="display:flex;gap:8px;align-items:center;font-family:'JetBrains Mono',monospace;font-size:11px;margin-bottom:16px">${parts.join('<span style="color:#4a5a7a">/</span>')}</nav>`;
}
...
function wrapPage(opts) {
  const { title, metaDescription, canonical, prev, next, jsonLd, breadcrumbHtml, h1, bodyHtml, noindex } = opts;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
