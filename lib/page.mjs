...
function renderFooter() {
  ...
  <div class="ac-footer-inner">
    <div>© 2026 AgentCanary · Market context layer for autonomous AI agents · Not financial advice</div>
...
function renderCollection(opts) {
  ...
  const jsonLd = {
    ...
    description: 'Market context layer for autonomous AI agents. Daily macro briefs with regime, risk, narrative & scenario state and transparent source trails.',
    breadcrumb: jsonLdBreadcrumb(breadcrumb),
  };

  return wrapPage({
    title: 'The Record — AgentCanary',
    metaDescription: opts.metaDescription || 'The Record — archived market state for AI agents. Daily regime, risk, narrative & scenario context with transparent source trails and open methodology.',
...
<section class="rc-masthead">
  <div class="rc-container">
    ...
    <p class="rc-masthead-sub">Market context layer for autonomous AI agents. Regime, risk, narrative & scenario state — 4× daily, permanently archived, with transparent source trails.</p>
  </div>
</section>
  ...
  return wrapPage({
    ...
    metaDescription: (() => {
      ...
      if (stats && Number.isFinite(stats.hit_rate_pct)) {
        return `${ticker} — regime, narrative & scenario context across ${mentions.length} AgentCanary brief${mentions.length === 1 ? '' : 's'} ${dateRange}. Per-brief permalinks; methodology + outcomes at /record/.`;
      }
  ...
  return wrapPage({
    ...
    metaDescription: (() => {
      const dateRange = mentions.length > 0
        ? `(${mentions[mentions.length-1].date} → ${mentions[0].date})`
        : '';
      return `${mentions.length} AgentCanary brief${mentions.length === 1 ? '' : 's'} published while macro regime was ${slug.toUpperCase()} ${dateRange}. Full archive with per-brief permalinks, scenario state, and transparent source trails.`;
    })(),
...
function renderBrief(opts) {
  ...
  return wrapPage({
    ...
    bodyHtml: (() => {
      ...
      const _weekendBannerHtml = (brief && brief.weekendBanner)  // weekend OR weekday pre-market
...
const _GENERIC_FOOTER = `<section class="ac-seo-footer"><div class="ac-seo-footer-inner">
  ...
  <p>AgentCanary is the market context layer for autonomous AI agents. It delivers fresh regime classifications, risk scores, narrative momentum, scenario state, and macro + crypto context via simple API + MCP — schema-stable, fail-soft, with a freshness envelope on every response. No KYC, wallet-billed in stablecoins.</p>
...
function _seoFooter(canonical) {
  ...
  <p>AgentCanary tracks ${t} as part of a broader macro + crypto context surface designed for autonomous agent consumption — schema-stable JSON via REST + MCP, freshness envelopes on every response, no KYC. Methodology and outcomes (including where our reads are weak) are published openly at <a href="/record/">/record/</a>.</p>
  ${_ACCESS_LINE}
</div></section>`;
  }
