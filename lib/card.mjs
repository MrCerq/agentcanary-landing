...
function renderPageTier(brief, slot, meta, opts) {
  ...
  // Page tier = Card visual + body in a <details> collapsed by default.
  // <details> is semantic HTML5; Google indexes the collapsed content
  // fine (no JS toggle dance needed), so this preserves SEO indexability
  // while reducing the visual weight of stacked briefs on a day page.
  return `<article class="ac-page" id="brief-${slot}-${brief.date || ''}" style="margin-bottom:32px">
  ${cardHtml}
  <details class="ac-page-body-wrap">
    <summary class="ac-page-body-toggle">Expand brief content ▾</summary>
    <div class="ac-page-body">${bodyHtml}</div>
  </details>
</article>`;
}
