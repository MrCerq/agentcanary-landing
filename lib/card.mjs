...
function renderBriefCardInner(brief, slot, meta, opts) {
  ...
  const showPills = !opts.omitPills && tagsHtml;
  return `<article class="ac-card" id="${escapeHtml(briefId)}">
    <div class="ac-card-main">
      ...
      ${showPills ? `<div class="ac-card-pills">${tagsHtml}</div>` : ''}
...
function renderBody(brief, opts) {
  ...
  let out = String(raw)
    .replace(/&amp;/g, '&')
    .replace(/<br\s*\/?>/gi, '\n')
    // Promote standalone <b>HEADING</b> lines (where the closing tag
    // is at end-of-line) to <h3> — semantic for SEO + AI crawlers.
    // Inline bold (e.g., <b>SCENARIO A</b> — content) is preserved
    // because the closing tag is followed by text, not newline.
    .replace(/<b>([^<\n]+)<\/b>\n/g, '<h3>$1</h3>')
    // Strip redundant newline after any remaining <b>X</b> at line-end
    // (legacy bodies with quirks).
    .replace(/<\/b>\n(?!\n|<b>|<h3>)/g, '</b>');
  // Strip emoji icons that leaked into the body (hegemony-score icon set
  // 👀 🚨 ⚠️ 🟢 + variation-selector). Render-time strip catches historical
  // briefs where the icon was emitted before the upstream fix.
  out = out.replace(/[\u{1F440}\u{1F6A8}\u{26A0}\u{1F7E2}]\uFE0F?\s*/gu, '');
  // Reposition orphan 'Crypto sentiment:' line that appears below KEY PRICES
  // in radar — move it directly under CRYPTO section header.
  const orphanRe = /\n\s*\n(Crypto sentiment:[^\n]+\.)\n/;
  if (orphanRe.test(out) && /<b>CRYPTO<\/b>/.test(out)) {
    const m = out.match(orphanRe);
    if (m) {
      const sentLine = m[1];
      out = out.replace(orphanRe, '\n');
      out = out.replace(/(<b>CRYPTO<\/b>\n[^<]+?)(\n\s*\n<b>)/, `$1\n${sentLine}$2`);
    }
  }
  // Entity link rewrite
