...
function renderBody(brief, opts) {
  ...
  let out = String(raw)
    .replace(/&amp;/g, '&')
    .replace(/<br\s*\/?>/gi, '\n')
    // Section headers <b>NAME</b> render display:block already — the
    // literal \n immediately after them creates a redundant blank line
    // under pre-wrap. Strip that \n unless the next line is another
    // <b> section header (inter-section blank line must be preserved).
    .replace(/<\/b>\n(?!\n|<b>)/g, '</b>');
